...
export default class Bot {
  _transport: types.PlainTransport;
  _dataProducer: types.DataProducer;
  _mapStreamIdPeer: Map<number, Object>;
  _udpSocket: any;
  _sctpSocket: any;
  _sendStream: any;
  //@ts-ignore
  static async create({ mediasoupRouter }) {
    ...
    await new Promise((resolve) => udpSocket.bind(0, "0.0.0.0", resolve));

    const localUdpPort = udpSocket.address().port;

    // Connect the mediasoup PlainTransport to the UDP socket port.
    await transport.connect({ ip: "0.0.0.0", port: localUdpPort });
