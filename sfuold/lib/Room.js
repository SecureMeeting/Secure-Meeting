			...
			case 'sendMessage':
				{
					...
					if (recieverPeerId === null) { //send to all
						for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })) {
							otherPeer.notify(
								'recievedMessage',
								{
			...
			case 'changeScreenShare':
				{
					...
					if (turnedOnScreenshare) {
						...
						this._usersSharingScreenPeerId.splice(this._usersSharingScreenPeerId.indexOf(peer.id, 1));
					}
					// Notify other joined Peers.
					for (const otherPeer of this._getJoinedPeers(/* { excludePeer: peer } */)) {

						if (this._usersSharingScreenPeerId.length != 0) { //tell other users that a user is screensharing
							otherPeer.notify(
								'activeScreenShare',
								{
									peerId: this._usersSharingScreenPeerId,
