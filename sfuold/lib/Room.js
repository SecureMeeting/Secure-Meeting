...
class Room extends EventEmitter {
  ...
  constructor({
    ...
    // array of Peer Id of the user in the room that is sharing their screen
    this._usersSharingScreenPeerId = [];

    // array of Peer Id of the user in the room that are raising their hand
    this._usersRaisingHand = [];

    // Keeps track of locked room status
    this._isRoomLocked = false;

    // The password of a locked room
    this._roomPassword = null;

    // The peerId of the host
    this.peerIdHost = null;
  ...
  async _handleProtooRequest(peer, request, accept, reject) {
    switch (request.method) {
      ...
      case "join": {
        ...
        //Tells the user that joined a room in session, whos screensharing
        if (this._usersSharingScreenPeerId.length != 0) {
          peer
            .notify("activeScreenShare", {
              peerId: this._usersSharingScreenPeerId,
            })
            .catch(() => {});
        } else {
          peer
        ...
        //tells the peer who the host is
        if (this.peerIdHost) {
          peer
            .notify("setRoomHost", {
              peerId: this.peerIdHost,
            })
            .catch(() => {});
        }

        // Tells the user that joined a room in session, if its locked

        peer
          .notify("changeLockedStatus", {
            status: this._isRoomLocked,
          })
          .catch(() => {});

        // Tells the user that joined a room in session, the public messages
        let publicMessages = [];
        for (var i = 0; i < this.messages.length; i++) {
          if (!this.messages[i].isPrivateMessage) {
            publicMessages.push(this.messages[i]);
          }
        }

        peer
          .notify("initialMessages", {
            initMessages: publicMessages,
          })
          .catch(() => {});
        ...
        //tell user they are not locked out
        peer
          .notify("peerIsLockedOut", {
            lockedOut: false,
          })
          .catch(() => {});
      ...
      case "raisedHand": {
        ...
        this._usersRaisingHand.push(peerId);

        for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })) {
          otherPeer
            .notify("peersRaisingHand", {
              peersRaisingHand: this._usersRaisingHand,
              peerName: peer.data.displayName,
            })
            .catch(() => {});
        }
      ...
      case "sendMessage": {
        ...
        const { recieverPeerId, senderName, message } = request.data;

        var Message = {
          recieverPeerId: recieverPeerId,
        ...
        if (recieverPeerId === null) {
          ...
          for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })) {
            if (otherPeer._id === recieverPeerId) {
              otherPeer
                .notify("recievedMessage", {
                  recieverPeerId: recieverPeerId,
                  senderName: senderName,
      ...
      case "unRaisedHand": {
        ...
        let indexOfPerson = -1;
        for (var i = 0; i < this._usersRaisingHand.length; i++) {
          if (this._usersRaisingHand[i] === peerId) {
            indexOfPerson = i;
            break;
          }
        }
        ...
        if (indexOfPerson != -1) {
          this._usersRaisingHand.splice(indexOfPerson, 1);
          for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })) {
            otherPeer
              .notify("peersRaisingHand", {
                peersRaisingHand: this._usersRaisingHand,
          ...
          throw new Error(`peer with id "${indexOfPerson}" not found`);
        }
      ...
      case "pauseSpecifcAudioProducer": {
        // Ensure the Peer is joined.
        if (!peer.data.joined) throw new Error("Peer not yet joined");
        ...
        for (const otherPeer of this._getJoinedPeers({ excludePeer: peer })) {
          if (otherPeer.id === peerId) {
            for (const [key, value] of otherPeer.data.producers.entries()) {
              if (value._data.kind === "audio") {
                const producer = value;
                await producer.pause();
              }
            }
            otherPeer
              .notify("remotelyMuted", {
                displayName: peer.data.displayName,
              })
              .catch(() => {});
          }
      ...
      case "changeLockedStatus": {
        ...
        if (this._isRoomLocked) {
          this._isRoomLocked = false;
        } else {
          this._isRoomLocked = true;
        }

        // Notify all joined joined Peers.
        for (const peer of this._getJoinedPeers()) {
          peer
            .notify("changeLockedStatus", {
              status: this._isRoomLocked,
      ...
      case "changeScreenShare": {
        // Ensure the Peer is joined.
        if (!peer.data.joined) throw new Error("Peer not yet joined");

        const { turnedOnScreenshare } = request.data;

        if (turnedOnScreenshare) {
          this._usersSharingScreenPeerId.push(peer.id); //used to allow the room to know who is screensharing for people who join into the call
        } else {
          this._usersSharingScreenPeerId.splice(
            this._usersSharingScreenPeerId.indexOf(peer.id, 1)
          );
        }
        // Notify other joined Peers.
        for (const otherPeer of this._getJoinedPeers(/* { excludePeer: peer } */)) {
          if (this._usersSharingScreenPeerId.length != 0) {
            //tell other users that a user is screensharing
            otherPeer
              .notify("activeScreenShare", {
                peerId: this._usersSharingScreenPeerId,
              })
